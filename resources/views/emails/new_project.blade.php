...
<head>
    ...
    <body style="font-family: arial; background-color: #F7F7F7; margin: 0px; padding: 0px;">
        ...
        <div style="width: 600px; margin:auto; background-color: #F7F7F7; padding-bottom: 30px; border-top-left-radius: 10px; border-top-right-radius: 10px; box-shadow: 0px 0px 10px 1px #00000021; margin-bottom: 30px;">
            ...
            <div style="width: 600px; margin:auto; background-color: #F7F7F7; padding-bottom: 30px; border-top-left-radius: 10px; border-top-right-radius: 10px; box-shadow: 0px 0px 10px 1px #00000021; margin-bottom: 30px;">
			    <div style="padding-left: 30px; padding-right: 30px; padding-top: 30px; text-align: left;">
			        <div style="font-size: 19px;color: #707070;line-height: 120%;margin-bottom: 30px;">
			            Hey <span style="color: #2e2e2e;">{{ ucfirst($receiver) }}</span>,
			            <br>
			            {{ $subject }}
			            <br>
			            <br>
			            {{ __('Please find the following details of the project') }}
			        </div>
			        <div style="font-size: 19px;color: #707070;line-height: 120%;>
			        	<span style="color: #2e2e2e;">{{ __('Project:') }}</span>
			        	<span>{{ $data['projectObj']->title }}</span>
			        </div>
			        <div style="font-size: 19px;color: #707070;line-height: 120%;>
			        	<span style="color: #2e2e2e;">{{ __('Project Id:') }}</span>
			        	<span>{{ $data['projectObj']->project_id }}</span>
			        </div>
			        <div style="font-size: 19px;color: #707070;line-height: 120%;>
			        	<span style="color: #2e2e2e;">{{ __('Due Date:') }}</span>
			        	<span>{{ date('d F, Y',strtotime($data['projectObj']->due_date)) }}</span>
			        </div>
			        <div style="font-size: 19px;color: #707070;line-height: 120%;>
			        	<span style="color: #2e2e2e;">{{ __('Client:') }}</span>
			        	<span>{{ $data['projectObj']->User->name }}</span>
			        </div>
			        <div style="width:290px; margin: auto;">
			            <a href="{{ url('project/'.$data['projectObj']->project_id) }}" style="font-size: 16px;text-transform: uppercase;letter-spacing: 1px;padding: 10px 0px;background: #73879C;text-decoration: none; color: #fff;border-radius: 5px;cursor: pointer;display: block; text-align: center; margin-top: 30px;">
			               {{ __('View Project') }}
			        	</a>
			   		</div>
